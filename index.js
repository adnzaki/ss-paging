...
function getData(options, callFromRunPaging) {
  if (callFromRunPaging === undefined) {
    callFromRunPaging = false
  }
  ...
  if (options.useHeader) {
    ...
    baseURL = ''
      ...
      .concat(state.limit, '/')
      .concat(state.offset, '/')
      .concat(state.orderBy, '/')
      .concat(state.searchBy, '/')
      .concat(state.sort)
  }
  ...
  fetch(requestURL, fetchOptions)
    ...
    .then(function (res) {
      ...
      create({
        ...
        linkNum:
          (_a = options.linkNum) !== null && _a !== undefined
            ? _a
            : state.linkNum,
        activeClass:
          (_b = options.activeClass) !== null && _b !== undefined
            ? _b
            : state.activeClass,
        linkClass:
          (_c = options.linkClass) !== null && _c !== undefined
            ? _c
            : state.linkClass,
        disabledClass:
          (_d = options.disabledClass) !== null && _d !== undefined
            ? _d
            : state.disabledClass,
      })
      ...
      if (options.afterRequest !== undefined) {
        if (!callFromRunPaging) {
          afterRequest.value = function () {
            return options.afterRequest()
          }
        }
...
function rowRange() {
  if (state.pageLinks.length === 0) {
    ...
    if (state.sentences[state.pagingLang] !== undefined) {
      returnedText = ''
        .concat(state.sentences[state.pagingLang].showRows, ' ')
        .concat(dataFrom.value, ' - \n                      ')
        .concat(dataTo.value, ' ')
        .concat(state.sentences[state.pagingLang].from, ' ')
        .concat(state.totalRows, ' \n                      ')
...
var usePagingStore = defineStore('sspaging', function () {
  return usePaging()
})
